define(function (require, exports, module) {
	...
	function doneItemHandler(event){
		...
		var db = qiao.db();
		qiao.query(db, 'select * from t_plan_day_todo where id=' + todoId, function(res){
			if(res.rows.length > 0){
				var data = res.rows.item(0);
				
				qiao.query(db, 'select max(id) mid from t_plan_day_done', function(res1){
					...
					qiao.update(db, 'insert into t_plan_day_done (id, plan_title, plan_content) values (' + (id+1) + ', "' + data.plan_title + '", "' + data.plan_content + '")');
					qiao.update(db, 'delete from t_plan_day_todo where id=' + todoId);
				});
			}
		});
	}
